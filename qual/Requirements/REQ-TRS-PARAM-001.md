...
A `FeatureDef` may declare typed `parameters:` (§9.7) — quantitative variability bound to concrete values in a selecting `Configuration` via `parameterBindings:` (a map keyed by `<FeatureDef qualified name>::<parameter name>`). When a feature model is present, the tool **shall** enforce the following single-level binding rules:
...
| `E222` | A `parameterBindings` key does not resolve to a declared parameter (malformed path, unknown `FeatureDef`, or undeclared parameter name) |
...
**Out of scope (not yet implemented):** two-level `bindTo:` parameter propagation (`E202`), circular `derivedFrom:` expression detection (`E207`), and cross-feature `parameterConstraints` (`E213`/`W014`).
